...
먼저 추적하려는 실제 물체를 충분히 정확하게 표현한 3D model이 필요합니다. Object Tracking에서는 이 3D model을 USDZ 파일로 준비합니다. Create ML은 USDZ를 바탕으로 현실에서 그 물체를 알아보는 데 필요한 정보를 학습하고, 결과를 `.referenceobject` 파일로 만듭니다.
...
Object Tracking에는 사용하는 동안 형태가 크게 바뀌지 않는 물체가 적합합니다. 접히거나 휘어지는 물체처럼 외형이 계속 달라지면 미리 준비한 3D model과 현실의 모습이 일치하지 않기 때문입니다.

### 현실 물체를 표현하는 USDZ

USDZ는 현실 물체의 모습을 3D 공간에 표현하는 파일입니다. 물체의 3D 형태를 이루는 geometry뿐 아니라 실제 크기, 방향, 표면의 색과 무늬를 표현하는 material과 texture 같은 정보를 담을 수 있습니다.

Object Tracking용 USDZ는 실제 물체와 충분히 잘 맞아야 합니다.

- **전체 형태**: 튀어나온 부분이나 빈 공간처럼 물체를 구분하는 구조가 반영되어야 합니다.
- **실제 크기**: 3D model의 가로, 높이, 깊이가 현실 물체와 맞아야 합니다.
- **방향과 원점**: model의 앞·뒤와 위·아래가 분명해야 하며, 이후 가상 콘텐츠를 배치할 기준을 이해할 수 있어야 합니다.
- **눈에 띄는 특징**: 손잡이, 모서리, 무늬처럼 물체를 구분하기 쉬운 특징이 빠지지 않아야 합니다.
- **재질과 표면**: 색, 무늬, 반사와 같은 외형이 현실 물체를 충분히 나타내야 합니다.

3D model이 현실 물체를 더 잘 표현할수록 추적 품질을 높이는 데 도움이 됩니다. 그렇다고 모든 model이 반드시 사진처럼 보여야 한다는 뜻은 아닙니다. 중요한 것은 추적 대상의 형태, 크기, 방향과 주요 외형이 실제 물체와 잘 맞는지입니다.

### Create ML이 만드는 Reference Object

Create ML은 앞에서 준비한 USDZ를 분석하고, Vision Pro가 현실에서 해당 물체의 위치와 방향을 찾는 데 사용할 Object Tracking model을 학습합니다.
...
Reference Object 안에는 입력 3D model이 함께 포함될 수 있습니다. Reality Composer Pro는 이를 반투명한 배치 가이드로 보여 줄 수 있지만, `.referenceobject`의 주된 역할은 화면에 보일 model을 제공하는 것이 아니라 현실 물체를 인식하고 추적하는 것입니다.
...
Object Anchor 아래에 가상 Entity를 배치하면, 현실 물체가 움직일 때 가상 콘텐츠도 같은 기준을 따라 움직입니다. Swift 코드에서 매 프레임 Transform을 직접 복사하지 않아도 RealityKit이 Reality Composer Pro에서 구성한 Entity의 부모-자식 구조, 즉 hierarchy를 사용해 콘텐츠를 정렬합니다.

### 가상 콘텐츠와 Animation

Object Anchor는 현실 물체를 따라가는 기준점입니다. 실제 화면에 보여 줄 가상 model은 그 아래에 별도의 Entity로 배치합니다.

완성 사례에서는 `ToyBiplane`과 `Pivot`이 `TreasureChestAnchor` 아래에 있습니다. `Pivot`은 별도의 기능 없이 Transform만 가진 Entity로, 추적된 보물상자 좌표계 안에서 비행기의 회전 중심을 제공합니다. Timeline은 이 기준을 이용해 비행기가 보물상자 주변을 돌도록 만듭니다.
